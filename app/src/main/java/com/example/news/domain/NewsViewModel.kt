...
class NewsViewModel @Inject constructor(
    private val viewModelRepository: NewsRepository): ViewModel(){
        private val _NewsData = MutableStateFlow(NewsState())
        val newsData : StateFlow<NewsState> = _NewsData
    ...
    fun getNewsData(category: String){
        viewModelScope.launch {
           try {
               Log.d("NEWS_API1", "Category: $category  ")
              ...
              _NewsData.value = _NewsData.value.copy(
               ...
               val result = viewModelRepository.getNews(category)

               result.onSuccess{ data ->
                   Log.d(
                       "NEWS_API",
                       "Success: ${data.results?.size}"
                   )
